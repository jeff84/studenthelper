...
{	
	function twitter_class()
	{
		...
		$this->searchURL = 'http://search.twitter.com/search.atom?lang=de&q=';
	...
	function getTweets($q, $limit=15)
	{
		...
		$response = file_get_contents($this->searchURL.urlencode($q));
		...
		if ($response !== FALSE)
		{
			...
			for($i=0; $i<count($xml->entry); $i++)
			{
				...
				<div id="tweet" class="tweet">
					...
					<div class="message">
						<span class="author"><a href="' . $account . '"  target="_blank">' . $name . '</a></span>: <br/>' . 
						$tweet . 
